<template>
	<view class="content">
		...
		<view class="record">
				<view class="record__item" v-for="(item,index) in list" :key="index" @click="goCaptureDetail" :data-id="item.id" :data-status="item.win_status" :data-winid="item.win_id">
						...
						<view class="record__item__info__details">
							<view class="record__item__info__details__left">
								...
								<view class="record__item__info__details__amount">
									参与次数：<text>{{item.userCount}}</text>次
								</view>
		...
		methods: {
			...
			goCaptureDetail(e){
				if(e.currentTarget.dataset.status == 0){
					uni.navigateTo({
							url: '/pages/address/list?id=' + e.currentTarget.dataset.winid +"&type=capture"
					});
				}else{
					uni.navigateTo({
