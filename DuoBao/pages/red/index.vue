	...
	<view id="red" class="page">
		...
		<view class="data-box">
			...
			<view class="end-box">
				<view class="end-box-l">
					...
					<text class="num">{{user.integral}}币</text>
				</view>
				<view class="end-box-r">
					...
					<text class="title">天天做任务领取金币或红包</text>
				</view>
			</view>
		</view>
		...
		<view class="task__bd">
			<view class="count-box">
				...
				<task-list ref="task" tag="index" :size="15" :isFilter="false"></task-list>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			onRed(item) {
				...
				if (item.status == 0) {
					if (parseInt(this.user.integral) > parseInt(item.integral)) {
						ajax.post({
