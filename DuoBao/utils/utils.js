		...
		ajax.get({
			...
			if (res.data.code == 200) {
				...
				for (let item of res.data.data) {
					if (typeof(ssy) == 'undefined' && item.id == 417 && !item.isComplete) {
						ssy = item;
					}
				...
				for (let item of res.data.data) {
					...
					if (typeof(ssy) != 'undefined' && item.tag !== 'aliad' && item.tag !== 'shareBubble') {
						let addData = Object.assign({}, ssy);
						if (item.tag == 'indexPopup') {
							addData.image = store.state.setUp.kxzb_redBg
						} else if (item.tag == 'indexFloatAd') {
							addData.image = store.state.setUp.kxzb_leftAd
						}
				...
				console.info(store.state.setUp)
