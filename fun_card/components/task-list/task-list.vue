		...
		<view class="task__body">
					...
					<view class="item__right">
						<view class="item__right__subtitle item__right__subtitle--gold">
							...
							<template v-if="item.reward_type=='4'">
								<view>+{{item.jfb}}<text>集分宝</text></view>
			...
			<uni-popup v-if="isinit" class="popups" ref="follows" type="center" :maskClick="false">
				...
				<view class="taskpopupscomplete__close">
					<image src="https://coubei.oss-cn-hangzhou.aliyuncs.com/uniapp/cancel.png" @click="completeClose" mode="widthFix"></image>
				</view>
			</uni-popup>
			...
			<uni-popup v-if="isinit" class="couponc" ref="coupon" type="top" :maskClick='false'>
				<view class="coupon" @click="this.$refs.coupon.close()">
					<image class="img" src="https://coubei.oss-cn-hangzhou.aliyuncs.com/uniapp/taskCoupon.png_xcxwep?v2" />
				</view>
			</uni-popup>
		</view>
		...
		methods: {
			...
			onTaskTap(task, tag = '') {
				...
				if (isLife && !task.isComplete) {
					// uni.navigateTo({
					// 	url: '/pages/attention/attention?tag='+this.tag
					// })
					this.lifeTask = task
					this.task = task
