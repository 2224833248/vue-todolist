...
module.exports = {
	...
	newJump(ad) {
		if (ad.type == 0) { // 无操作
			return '';
		} else if (ad.type == 101) { // 跳转页面
			uni.navigateTo({
				url: ad.url
			})
		} else if (ad.type == 102) { // 跳转插件
			uni.navigateTo({
				url: ad.url
			})
		} else if (ad.type == 103) { // 内容部打开网页
			uni.navigateTo({
				url: "/pages/webview/webview?url=" + ad.url
			})
		} else if (ad.type == 201) { // 跳转小程序
			let query = ad.url.substr(ad.url.indexOf('?') + 1);
		...
		} else if (ad.type == 301) { //	跳转生活号
			my.ap.navigateToAlipayPage({
				path: ad.url
			})
		} else if (ad.type == 302) { //通用浏览器模式
			my.ap.navigateToAlipayPage({
				path: encodeURIComponent(
