<template v-if="Object.keys(taskList).length > 0">
	<view>
		<view class="task__body">
				 ...
				 :key="index">
					<view class="item__right">
						<form @submit="onSubmit" :report-submit="true">
							<button formType="submit" v-if="item.isComplete == false" class="item__right__button" @click="onTaskTap(item)">{{item.button_text=='' || item.button_text==null?'去完成':item.button_text}}</button>
							<button v-else class="item__right__button" @click="onTaskTap(item)">去看看</button>
						</form>
					</view>
					<view class="item__left">
						<view class="item__info">
							<view class="item__info__hd">
								...
								<view class="item__info__hd__subtitle item__info__hd__subtitle--gold">
									<template v-if="item.reward_type=='1'">
										<view>+{{item.integral}}</view>
										<image style="width: 28rpx; margin-left:10rpx ;" :lazy-load="true" src="/static/index/gold.png" mode="widthFix"></image>
									</template>
									...
									<template v-if="item.reward_type=='3'">
										...
										<image style="width: 28rpx; margin-left:10rpx ;" :lazy-load="true" src="/static/index/redbag.png" mode="widthFix"></image>
									</template>
									<template v-if="item.reward_type=='4'">
										<view>+{{item.jfb}}集分宝</view>
										<image style="width: 28rpx; margin-left:10rpx ;" :lazy-load="true" src="/static/index/jfb.png" mode="widthFix"></image>
									</template>
									<template v-if="item.reward_type=='5'">
										<view>+{{item.wish_bean}}</view>
										<image style="width: 28rpx; margin-left:10rpx ;" :lazy-load="true" src="/static/index/luckdpu.png" mode="widthFix"></image>
									</template>
								</view>
							</view>
			...
			<uni-popup v-if="isinit" class="popups" ref="follows" type="center" :maskClick="false">
				<view class="popups">
					<view class="taskguanzhucomplete">
						...
						<!-- <button class="taskguanzhucomplete__ft" type="default" open-type="lifestyle" publicId="lifeTask.appid">立即关注</button>
			...
			<!-- <uni-popup class="popups" ref="life" type="center">
			<view class="popups">
				<view class="life">
					<view class="content">
						<template v-if="lifeTask.reward_type=='1'">
							<text class="title">+{{lifeTask.integral}}金币</text>
							<text class="descr">成功关注生活号+{{lifeTask.integral}}金币</text>
						</template>
						<template v-if="lifeTask.reward_type=='2'">
							<text class="title">+{{lifeTask.step}}步</text>
							<text class="descr">成功关注生活号+{{lifeTask.step}}步</text>
						</template>
						<template v-if="lifeTask.reward_type=='3'">
							<text class="title">+{{lifeTask.money}}元</text>
							<text class="descr">成功关注生活号+{{lifeTask.money}}元</text>
						</template>

					</view>
					<life-follow class="btn" :sceneId="lifeTask.appid" @close.native="lifeClose" :checkFollow="false" />
				</view>
			</view>
		</uni-popup> -->

			<uni-popup v-if="isinit" class="desktop" ref="desktop" type="top" :maskClick='false'>
				<image class="desktop__img" src="https://coubei.oss-cn-hangzhou.aliyuncs.com/upload/f510e23d073d307697b845e84f4398f1.png_xcxwep"
				 mode="widthFix"></image>
			...
			<uni-popup v-if="isinit" class="desktop" ref="aliindex" type="top" :maskClick='false'>
				<image class="desktop__img" src="https://coubei.oss-cn-hangzhou.aliyuncs.com/upload/7a6b11b3d1c229a6ff6be7bd9c2bbaa1.png_xcxwep"
				 mode="widthFix"></image>
				<view class="desktop__button" @click="this.$refs.aliindex.close()">
					我知道了
				</view>
			</uni-popup>
			<uni-popup v-if="isinit" class="couponc" ref="coupon" type="top" :maskClick='false'>
				<view class="coupon" @click="this.$refs.coupon.close()">
					<image class="img" src="https://coubei.oss-cn-hangzhou.aliyuncs.com/uniapp/taskCoupon.png_xcxwep?v2" mode="widthFix" />
				</view>
			</uni-popup>
		</view>
		...
		methods: {
			...
			postTask(isLife, isNoSet = false, tag = '') {
				...
				let isEmpty = this.activeTask.integral == 0 && this.activeTask.step == 0 && this.activeTask.jfb == 0 && this.activeTask
					.money == 0 && this.activeTask.wish_bean == 0;
			...
			taskList() {
				...
				if (this.tag == '@all') {
					...
					for (let item of this.$store.state.taskList) {
						if (item.key.indexOf('ubble') !== -1 || item.key.indexOf('aliad') !== -1 || item.key == 'funcardCardEnroll') {
							continue;
						}
