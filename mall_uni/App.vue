<script>
	...
	App = function(config) {
		...
		config.umengConfig = {
			appKey: '5f59fb03b4739632429cd55b', //由友盟分配的APP_KEY
	...
	export default {
		onLaunch(option) {
			...
			// 跳转其他小程序
			if (typeof(option.query) == 'object' && typeof option.query.jid == "string"){
				ajax.get({
					url: 'Base/getJump',
					data:{
						id:option.query.jid
					...
					if (res.data.code == 200) {
						let jump_url = res.data.data
						my.navigateToMiniProgram({
							appId: jump_url.appid,
							path: jump_url.path,
							extraData: jump_url.extraData
						});
					}
				})
			}
